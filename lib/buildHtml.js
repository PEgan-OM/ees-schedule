...
// "2026-06-03" -> "Wed, Jun 3" (parsed as UTC so the date doesn't shift).
// The date portion is extracted with a regex rather than concatenating a time
// suffix, so any YYYY-MM-DD[...] input (bare date, full ISO timestamp, trailing
// Z) still yields a weekday name. This is what stops the day-of-week header
// from silently collapsing to a bare date.
...
// "08:00" -> "8:00 AM". Leaves non-time strings untouched.
function to12(hm) {
...
// "08:00–16:30" -> "8:00 AM–4:30 PM" (handles en-dash or hyphen).
function fmtTimeRange(t) {
  if (!t) return t;
  return t.split(/(–|-)/).map((s) => (s === '–' || s === '-' ? s : to12(s))).join('');
}

// Start time (minutes from midnight) used to sort a day's blocks chronologically.
function startMinutes(raw) {
  const time = (raw.split(' | ')[3] || '').trim();
  const m = /^(\d{1,2}):(\d{2})/.exec(time);
  return m ? +m[1] * 60 + +m[2] : Infinity; // untimed blocks sort to the bottom
}
...
module.exports = { buildHtml, colorFor, weekGrid, fmtDate, fmtTimeRange, startMinutes };
